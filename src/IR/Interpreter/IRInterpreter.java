...
public class IRInterpreter {
   ...
   //     IRInterpreter vm = new IRInterpreter(System.in, ssa, new DataInputStream(System.in), new PrintStream(System.out));
        //if (ssa)
    ...
    private void runInstruction() throws RuntimeError, IOException {
        ...
        switch (curInst.operator) {
            case "load":
                ...
                curInst.size = 4;
                int res = 0;
                for (int i = 0; i < curInst.size; ++i)
                    res = (res << 8) | memoryRead(addr + i);
            ...
            case "call":
                ...
                switch (curInst.op1) {
                    ...
                    case "printInt": {
                        int x = readSrc(curInst.args.get(0));
                        data_out.print(x);
                        return;
                    }
                    case "printlnInt": {
                    ...
                    case "array.size": {
                        int aaddr = readSrc(curInst.args.get(0));
                        curInst.size = 4;
                        int rres = 0;
                        for (int i = 0; i < curInst.size; ++i)
                            rres = (rres << 8) | memoryRead(aaddr + i);
                        registerWrite(curInst.dest, rres);
                        return;
                    }
                    default:
                        break;
    ...
    private void runFunction(Function func) throws RuntimeError, IOException {
        //if (func.name.equals("vector.scalarInPlaceMultiply"))
          //  printAnything(123);
...
//                    }
//                    case "array.size": {
//                        long aaddr = readSrc(curInst.args.get(0));
//                        curInst.size = 8;
//                        int rres = 0;
//                        for (int i = 0; i < curInst.size; ++i)
//                            rres = (rres << 8) | memoryRead(aaddr + i);
